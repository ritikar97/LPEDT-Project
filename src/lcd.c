...
void displayInit()
{
    ...
    //
    gpioSensorEnable(); // we need SENSOR_ENABLE=1 which is tied to DISP_ENABLE
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(TIME_1HZ, 0, false);
	  if (timer_response != SL_STATUS_OK)
	   {
	      LOG_ERROR("sl_bt_system_set_soft_timer() returned error with status = 0x%04x", timer_response);
     }
