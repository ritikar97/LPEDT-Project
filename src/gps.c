...
//                       | (USER_RX_LOCATION << _USART_ROUTELOC0_RXLOC_SHIFT);
  ...
  USART_IntEnable(USART0, USART_IEN_RXFULL);
...
sl_status_t gps_send_cmd(char *cmd)
{
  ...
  if(!handle.tx_busy) {
    ...
    handle.tx_busy = true;
    USART_IntEnable(USART0, USART_IF_RXFULL);
...
//  if(flags & USART_IF_RXFULL) {
//      process_char((char)(USART0->RXDATA));
//  }
//
...
//    USART0->TXDATA = handle.tx_buffer_ptr[handle.tx_buffer_idx++];
//  } else
//  /*
...
//   */
//  {
//    USART_IntDisable(USART0, USART_IF_TXC); // Disable TX FIFO Level Interrupt
